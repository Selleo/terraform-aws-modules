module "ci_iam" {
  ...
  name = "name-staging-ci" # TODO adjust name to your project
...
data "aws_iam_policy_document" "ci_user" {
  statement {
    ...
    resources = ["arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/name/staging/api/*"] # TODO adjust name to your project
  }
  statement {
    effect = "Allow"
    actions = [
